...
def load_targets_data():
    ...
    try:
        # ¡AQUÍ ESTÁ LA MAGIA! skiprows=1 hace que ignore la fila 1 y use la 2 como títulos.
...
with tab_targets:
    ...
    if df_targets is not None and not df_targets.empty:
        if 'Date' in df_targets.columns and 'Target' in df_targets.columns:
            ...
            st.info(f"🕵️ **Las columnas que estoy leyendo en la fila 2 son:** \n\n `{cols_found}`")
    else:
        st.info("⏳ Cargando datos de Targets o el documento está vacío...")import streamlit as st
import pandas as pd
import plotly.express as px
